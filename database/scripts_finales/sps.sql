...
DROP PROCEDURE IF EXISTS sp_noticia_readByIdUsuario;
DROP PROCEDURE IF EXISTS sp_noticia_readByFavoritasIdUsuario;
...
DROP PROCEDURE IF EXISTS sp_valoracion_readByUsuarioComentario;
DROP PROCEDURE IF EXISTS sp_valoracion_readByUsuarioNoticia;
...
DELIMITER //
CREATE PROCEDURE sp_noticia_readByIdUsuario #call sp_noticia_readByIdUsuario (1)
(
	IN p_id_usuario  INT
)
BEGIN
...
DELIMITER //
CREATE PROCEDURE sp_noticia_readByFavoritasIdUsuario #call sp_noticia_readByFavoritasIdUsuario (2)
(
	IN p_id_usuario  INT
)
BEGIN
	...
	SELECT 		n.id_noticia, 
                ...
                IFNULL((SELECT m.ruta FROM noticia_multimedia m WHERE m.id_noticia = n.id_noticia LIMIT 1), '') AS thumbnail
	FROM 		noticia n
	INNER JOIN	valoracion v
				ON n.id_noticia = v.id_noticia
    WHERE		v.id_usuario = p_id_usuario
				AND n.aprobada = true
				AND v.favorito_noticia = true
                AND v.tipo_valoracion = 1
    ORDER BY    n.id_noticia DESC;
...
BEGIN 
	INSERT INTO comentario	(fecha_hora, descripcion, id_usuario, id_noticia, id_comentario_padre)
	VALUES 					(NOW(), p_descripcion, p_id_usuario, p_id_noticia, p_id_comentario_padre);
END //
...
CREATE PROCEDURE sp_comentario_readBynoticia -- CALL sp_comentario_readBynoticia(2);
(
	IN p_id_noticia		INT
)
BEGIN
	...
	SELECT 	 	c.id_comentario,
                ...
                c.id_usuario,
                c.id_noticia,
                IF(u.nick IS NOT NULL, u.nick, u.correo) AS nick,
                u.ruta_imagen AS avatar,
                c.id_comentario_padre,
				(SELECT COUNT(*) FROM valoracion sqv WHERE sqv.id_comentario = c.id_comentario AND sqv.valoracion_comentario = true) AS votos_positivos,
				(SELECT COUNT(*) FROM valoracion sqv WHERE sqv.id_comentario = c.id_comentario AND sqv.valoracion_comentario = false) AS votos_negativos
    FROM		comentario c
    ...
    WHERE		c.id_noticia = p_id_noticia
				AND (c.id_comentario_padre IS NULL OR c.id_comentario_padre = 0)
...
(
    ...
    IN p_tipo_valoracion		INT
)
BEGIN 
	INSERT INTO valoracion(id_noticia, id_comentario, id_usuario, favorito_noticia,
							valoracion_noticia, valoracion_comentario, fecha_hora, tipo_valoracion)
		VALUES 	(p_id_noticia, p_id_comentario, p_id_usuario, p_favorito_noticia,
					p_valoracion_noticia, p_valoracion_comentario, NOW(), p_tipo_valoracion);
END //
...
CREATE PROCEDURE sp_valoracion_readByUsuarioComentario -- call sp_valoracion_readByUsuarioComentario (2);
(
	IN p_id_usuario		 INT,
	IN p_id_comentario 		INT
)
BEGIN 
	...
	SELECT 		v.id_valoracion,
                ...
                v.id_usuario,
                v.favorito_noticia,
                v.valoracion_noticia,
                v.valoracion_comentario,
                v.tipo_valoracion,
                v.fecha_hora
    FROM 		valoracion v
    WHERE 		v.id_comentario = p_id_comentario
				AND v.id_usuario = p_id_usuario
                AND v.tipo_valoracion = 2
    LIMIT		1;
...
CREATE PROCEDURE sp_valoracion_readByUsuarioNoticia -- call sp_valoracion_readByUsuarioNoticia (1, 2);
(
	IN p_id_usuario INT,
	IN p_id_noticia INT
...
BEGIN 
	...
	SET			id_noticia = p_id_noticia,
                ...
                tipo_valoracion = p_tipo_valoracion
